...
def ventana_login(root):
    ...
    def iniciar_sesion():
        ...
        if resultado:
            ...
            print(sesion.usuario_actual)

            messagebox.showinfo("Bienvenido", f"Se ha logueado al Sistema de Gestión {usuario}")
          # 🔥 habilitar menú
            root.config(menu=login)
            login.destroy()
            root.deiconify()
        else:
            messagebox.showerror("Error", "Usuario o contraseña incorrectos o no está registrado")
